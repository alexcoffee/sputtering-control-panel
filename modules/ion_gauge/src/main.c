...
static pressure_display_unit_t pressure_unit_from_switch_state(bool switch_enable_active, bool switch_degas_active) {
    if (switch_enable_active && !switch_degas_active) {
...
static const scp_gpio_assignment_t g_gpio_assignments[] = {
    ...
    {SCP_GPIO_SIGNAL_SWITCH_ENABLE, 13},
    {SCP_GPIO_SIGNAL_SWITCH_DEGAS, 14},
};
...
int main(void) {
    ...
    if (0
        ...
        || !scp_pico_gpio_map_find_pin(&gpio_map, SCP_GPIO_SIGNAL_SWITCH_DEGAS, &switch_degas_gpio)) {
    ...
    gpio_init(switch_degas_gpio);
    gpio_set_dir(switch_degas_gpio, GPIO_IN);
    gpio_pull_up(switch_degas_gpio);
    ...
    while (true) {
        ...
        if (absolute_time_diff_us(now, next_sensor_sample) <= 0) {
            ...
            const bool switch_enable_active = !gpio_get(switch_enable_gpio);
            const bool switch_degas_active = !gpio_get(switch_degas_gpio);
            const pressure_display_unit_t display_unit =
                pressure_unit_from_switch_state(switch_enable_active, switch_degas_active);
